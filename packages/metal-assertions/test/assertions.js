...
describe('assertions', () => {
	...
	describe('assertPromise', () => {
		it('should throw error when it\'s not a promise', () => {
			assert.throws(() => {
				assertPromise(null, 'message');
			}, /message/);
		});

		it('should not throw error when it\'s a promise', () => {
			const p = (fail = false) => {
				return new CancellablePromise((resolve, reject) => {
					fail ? reject(fail) : resolve(fail);
				});
			};

			assert.doesNotThrow(() => {
				assertPromise(p(), 'message');
			});
		});
	});
